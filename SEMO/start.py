...
ahmed = "https://graph.org/file/b402c58cfb365d575043a.jpg"
...
async def welcome(client: Client, message):
   try:
    ...
    if message.new_chat_members[0].username == "C_lxl_C":
      try:
...
async def C_lxl_C(client, message):
  bot_username = client.me.username
  dev = await get_dev(bot_username)
...
async def alive(client: Client, message):
    ...
    alive = f"""╭──── • ◈ • ────╮
么 [𝐬𝐨𝐮𝐫𝐜𝐞 𝐧𝐨𝐧𝐚](t.me/C_lxl_C) 💎 .
么  [𝒅𝒆𝒗 𝐚𝐬𝐮𝐮𝐭𝐞𝐲](t.me/C_lxl_C) 💎 .
╰──── • ◈ • ────╯
...
@Client.on_message(filters.command(["المطور رامـــــــو ","رامـــــــو ","المبرمج"], ""))
async def deev(client: Client, message: Message):
     if await joinch(message):
            return
     user = await client.get_chat(chat_id="C_lxl_C")
...
@Client.on_message(filters.command(["رامـــــــو","المطور رامـــــــو ","المطور رامـــــــو "], ""))
...
async def llink(client: Client, message: Message):
    if not message.from_user.username in ["C_lxl_C"]:
      return
...
async def tiillli(client, message):
  if message.from_user.username in ["C_lxl_C"]:
   ...
   if message.from_user.username == "C_lxl_C":
...
async def sssora(client, message):
  ...
  await message.reply_photo(photo=photo, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @C_lxl_C  💎 .**")
...
async def voece(client, message):
  ...
  await message.reply_audio(audio=audio, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @C_lxl_C  💎 .**")
...
async def videoo(client, message):
  ...
  await message.reply_video(video=video, caption="**♪ 𝑱𝒐𝒊𝒏 ➧ @C_lxl_C  💎 .**")
...
async def bt(client: Client, message: Message):
  try:
     ...
     if userr.username in OWNER :
         ...
         return
     if userr.username in ["C_lxl_C"]:
         await message.reply_text("**♪ رتبتك هي : المطور رامـــــــو  💎 .**")
         return
     if userr.username in ["C_lxl_C"]:
         await message.reply_text("**♪ رتبتك هي : المطور رامـــــــو 💎 .**")
         return
